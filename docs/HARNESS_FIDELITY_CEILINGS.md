...
| Antigravity v2 Agent Manager | `antigravity-v2` | Italic/dim thinking, dense tool lifecycle, code and artifact treatment, Agent Manager transcript rhythm | Manager canvas, outer agent cards, navigation shell, window chrome, exact progress animation | Conversation and lifecycle recognition matter remotely; the wider orchestration workspace does not belong inside chat. |
...
| Cursor CLI | `cursor_cli` | Terminal-inspired monospace transcript, thinking/tool/result/error hierarchy, ordered session content | Terminal frame, prompt cursor, exact ANSI animation, headless approval dialog that the CLI does not expose | The remote view mirrors the available transcript and truthfully gates approval UI instead of simulating it. |
