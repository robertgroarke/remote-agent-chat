...
function renderPlainTextCode(rawText) {
  ...
  return lines.map((line, i) =>
    `<span class="code-line"><span class="code-line-num">${i + 1}</span>${renderInlineFileRefs(line)}</span>`
  ).join('');
}
...
codeRenderer.code = function(code, infostring) {
  ...
  const filepathHtml = filepath
    ? `<button class="diff-filepath" title="Open file preview" data-copy-path="${escapeAttr(filepath)}" data-open-path="${escapeAttr(filepath)}">${escapeHtml(filepath)}</button>`
    : '';
...
function renderToolSection(name, text, index) {
  ...
  // Default to expanded tool sections so remote users can read the full contents
  // without needing to click through collapsed summaries.
  ...
  // Keep only empty sections collapsed by default. Everything with readable content
  // should print inline, including Codex Desktop Bash/Edit summaries and bodies.
