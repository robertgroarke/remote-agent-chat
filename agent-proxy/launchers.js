...
function targetMatchesAgent(target, agentType) {
  ...
  if (agentType === 'continue_yolo') {
    return target.type === 'iframe' && (url.includes('continue-yolo') || url.includes('continue.continue-yolo'));
  }
...
async function executeNewSessionCommand(port, agentType, workspacePath) {
  ...
  const commandMap = {
    ...
    continue_yolo: 'continueYolo.editor.newSession',
  };
    ...
    try {
      ...
      if (input) {
        ...
        const cmdLabel = agentType === 'claude' ? 'Claude Code: New Session'
                       ...
                       : agentType === 'continue_yolo' ? 'Continue YOLO: New Editor Session'
                       : 'New Chat';
...
async function launchSession({ agentType, port, sessions, requestId, workspacePath, onSuccess, onFailure }) {
  ...
  // Continue YOLO opens in editor tabs and can be created from the workbench
  // command even when no existing YOLO session is open.
  ...
  if (existingSession) {
    ...
    if (!clicked && (agentType === 'claude' || agentType === 'codex' || agentType === 'gemini' || agentType === 'continue_yolo')) {
  ...
  } else if (agentType === 'claude' || agentType === 'continue_yolo' || agentType === 'antigravity' || agentType === 'antigravity_panel') {
    // No existing session — try command palette on existing workbench first
    let launched = false;
    if (agentType === 'claude' || agentType === 'continue_yolo') {
      try {
        const cmdResult = await executeNewSessionCommand(port, agentType, workspacePath);
