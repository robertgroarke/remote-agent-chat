...
const CDP_PORTS = (process.env.CDP_PORTS || process.env.CDP_PORT || '9223,9228,9226')
  .split(',')
...
function parseDurationEnv(name, defaultMs) {
  const value = Number(process.env[name]);
  return Number.isFinite(value) && value >= 0 ? value : defaultMs;
}

const STARTUP_GRACE_MS = parseDurationEnv('RESCUE_STARTUP_GRACE_MS', 45000);
const STARTUP_POLL_MS = parseDurationEnv('RESCUE_STARTUP_POLL_MS', 3000);
const WATCHDOG_POLL_MS = parseDurationEnv('RESCUE_WATCHDOG_POLL_MS', 15000);
...
async function waitForMainProxyConnected(timeoutMs) {
  const deadline = Date.now() + timeoutMs;
  while (true) {
    if (await mainProxyAlreadyConnected()) return true;
    const remainingMs = deadline - Date.now();
    if (remainingMs <= 0) return false;
    await sleep(Math.min(STARTUP_POLL_MS, remainingMs));
  }
}
...
async function main() {
  if (STARTUP_GRACE_MS > 0) {
    console.log(`[rescue-proxy] Waiting up to ${STARTUP_GRACE_MS}ms for main proxy at ${MAIN_HEALTH_URL} before activating rescue.`);
  }
  if (await waitForMainProxyConnected(STARTUP_GRACE_MS)) {
    console.log(`[rescue-proxy] Main proxy is connected at ${MAIN_HEALTH_URL}; rescue proxy exiting.`);
    return;
  }
  console.log(`[rescue-proxy] Main proxy still unavailable after startup grace; activating rescue proxy.`);
