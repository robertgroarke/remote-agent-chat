...
export default function PermissionPrompt({ prompt, agentType, onChoice }) {
  ...
  const ready = questions.every(question => {
    ...
    return selected.every(id => {
      const choice = question.choices.find((item, index) => idFor(item, index) === id);
  ...
  const submit = () => {
    if (!ready || submittingChoiceId) return;
    const answers = questions.map(question => {
      const choiceIds = selectedFor(question);
      const other = question.choices.find((choice, index) => choice.requires_text && choiceIds.includes(idFor(choice, index)));
      const otherId = other ? idFor(other, question.choices.indexOf(other)) : null;
      return {
  ...
  return (
    <View style={[s.container, claudeAction && s.claudeContainer]}>
      {claudeAction ? (
        ...
        <View style={s.header}>
          ...
          <Text style={s.title} numberOfLines={2}>{block?.label || prompt.title || (structured ? 'Question' : 'Permission Required')}</Text>
        </View>
      )}

      {claudeAction && !!prompt.command && <Text style={s.claudeCommand}>{prompt.command}</Text>}
      {!!(claudeAction ? prompt.description : (block?.content || prompt.description || prompt.prompt_text || prompt.message)) && (
        <ScrollView style={s.descScroll} nestedScrollEnabled>
          <Text style={[s.desc, claudeAction && s.claudeDesc]}>
            {claudeAction ? prompt.description : (block?.content || prompt.description || prompt.prompt_text || prompt.message)}
          </Text>
        </ScrollView>
      )}
      ...
      <View style={[s.choices, structured && s.questionList, claudeAction && s.claudeChoices]}>
        {structured ? questions.map((question, questionIndex) => (
          <View style={s.question} key={question.question_id || questionIndex}>
            <Text style={s.questionTitle}>{question.label || `Question ${questionIndex + 1}`}</Text>
            {!!question.message && <Text style={s.questionMessage}>{question.message}</Text>}
            {question.choices.map((choice, index) => {
              ...
              return (
                <View key={id} style={s.questionOption}>
                  <TouchableOpacity
                    style={[s.choiceBtn, s.questionChoice, selected && s.choiceSelected]}
                    activeOpacity={0.75}
                    disabled={!!submittingChoiceId}
                    onPress={() => toggle(question, id)}
                    accessibilityRole={question.multi_select ? 'checkbox' : 'radio'}
                    accessibilityState={{ disabled: !!submittingChoiceId, checked: selected }}
                  >
                    <Text style={s.choiceMarker}>{question.multi_select ? (selected ? '✓' : '□') : (selected ? '●' : '○')}</Text>
                    <View style={s.choiceCopy}>
                      <Text style={s.choiceText}>{choice.label || choice.title || choice.text || id}</Text>
      ...
      {structured && (
        <TouchableOpacity
          style={[s.submitBtn, (!ready || submittingChoiceId) && s.submitDisabled]}
          disabled={!ready || !!submittingChoiceId}
          onPress={submit}
          accessibilityRole="button"
          accessibilityState={{ disabled: !ready || !!submittingChoiceId, busy: !!submittingChoiceId }}
        >
          <Text style={s.submitText}>{submittingChoiceId ? 'Sending…' : (prompt.submit_label || 'Submit answers')}</Text>
        </TouchableOpacity>
      )}
    </View>
  );
...
const s = StyleSheet.create({
  ...
  choiceBtn: { borderRadius: 8, paddingVertical: 10, paddingHorizontal: 16, borderWidth: 1 },
  ...
  otherInput: { marginTop: 6, color: '#cdd9e5', backgroundColor: '#0d1117', borderWidth: 1, borderColor: '#30363d', borderRadius: 7, paddingHorizontal: 10, paddingVertical: 8 },
  ...
  submitBtn: { marginTop: 12, backgroundColor: '#1f6feb', borderRadius: 8, paddingVertical: 10, alignItems: 'center' },
