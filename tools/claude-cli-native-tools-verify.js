...
const { processAlive, workspaceSnapshot } = require('./claude-cli-native-conversation-e2e');
const { readToolTurn } = require('./claude-cli-native-tools-e2e');
...
function main(argv = process.argv.slice(2)) {
  ...
  assert.equal(source.ok, true, 'Source native tools result is not green');
  assert.equal(source.agent, 'claude_cli');
  assert.deepStrictEqual(source.native_surfaces, ['tool_call', 'tool_result', 'plan']);
  ...
  assert.deepStrictEqual(source.tools_available, ['Read', 'TaskCreate', 'TaskUpdate', 'TaskList']);
  assert.match(source.session_id, /^[0-9a-f-]{36}$/i);
  assert.match(source.marker, /^RAC_CLAUDE_NATIVE_TOOLS_[0-9A-F]{8}$/);
  assert.match(source.active_frame_text, /Reading\s+1\s+file/i);
  assert.match(source.active_frame_text, /Read fixture/);
  assert.match(source.active_frame_text, /Report token/);
  assert.match(source.active_frame_text, /esc\s+to\s+interrupt/i);
  assert(source.settled_frame_text.includes(source.marker));
  assert.match(source.settled_frame_text, /2 tasks \(2 done, 0 open\)/);
  assert.match(source.settled_frame_text, /Read fixture/);
  assert.match(source.settled_frame_text, /Report token/);
  ...
  assert.equal(source.safety.tools_invoked, 8);
  assert.deepStrictEqual(source.safety.allowed_tool_names, ['Read', 'TaskCreate', 'TaskUpdate', 'TaskList']);
  ...
  const fixtureBytes = fs.readFileSync(source.fixture.path);
  assert.equal(sha256(fixtureBytes), source.fixture.sha256);
  assert.equal(fixtureBytes.length, source.fixture.bytes);
  assert.equal(source.fixture.unchanged, true);
  ...
  const completed = readToolTurn(archivePath, source.marker);
  assert(completed, 'Native archive lost its task/read calls, results, assistant, or turn boundary');
  assert.equal(completed.toolUses.length, 8);
  assert.equal(completed.toolResults.length, 8);
  assert(completed.toolResults.every(result => result.is_error === false));
  assert.deepStrictEqual([...new Set(completed.toolUses.map(tool => tool.name))],
    ['TaskCreate', 'TaskUpdate', 'Read', 'TaskList']);
  assert.equal(completed.assistantText, source.native_assistant_text);
  assert.equal(completed.assistantStopReason, 'end_turn');

  const summary = claudeCli.readSessionSummary(archivePath);
  const blocks = summary.messages.flatMap(message => message.content_blocks || []);
  const plans = blocks.filter(block => block.type === 'plan');
  assert.equal(plans.length, 1, 'Current Task operations must reduce to one plan block');
  assert.equal(plans[0].title, '2 tasks (2 done, 0 open)');
  assert.deepStrictEqual(plans[0].tasks.map(task => ({ id: task.id, step: task.step, status: task.status })), [
  ...
  'Task operations must not fan out into generic tool cards');

  for (const surface of ['active', 'settled']) {
  ...
  const output = {
    ...
    tool_uses: completed.toolUses.length,
    tool_results: completed.toolResults.length,
    parsed_blocks: blocks.map(block => ({ type: block.type, title: block.title || null, tool_name: block.tool_name || null })),
    ...
    safety: { sends: 0, controls: 0, focus_actions: 0, visible_windows_opened: 0 },
  };
...
if (require.main === module) {
  try {
    ...
    console.error(`Claude CLI native tools verification: FAIL (${error.stack || error.message})`);
    process.exitCode = 1;
  }
}
