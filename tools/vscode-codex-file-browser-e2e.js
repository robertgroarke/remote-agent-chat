  ...
  await new Promise((resolve, reject) => {
    ...
    ws.once('open', () => {
      ...
      ws.send(JSON.stringify({
        ...
        protocol_version: 1,
        peer_role: 'browser',
        client_name: 'vscode-codex-file-browser-e2e',
      }));
      resolve();
    });
...
function storedCodexSession(sessionId) {
  ...
  assert(matches[0].target_id, 'Guarded Codex session has no durable target_id');
  return matches[0];
}

async function requestConfig(relay, sessionId) {
  const requestId = `codex-files-cfg-${crypto.randomBytes(4).toString('hex')}`;
  ...
  return waitFor(
    ...
    30000,
    'Codex file-browser config',
  );
}

async function requestFileAction(relay, sessionId, type, requestPath, options = {}) {
  const requestId = `codex-files-${crypto.randomBytes(4).toString('hex')}`;
  const start = relay.messages.length;
...
async function main(argv = process.argv.slice(2)) {
  ...
  guard.assertUpdatesDisabled('VS Code Codex file-browser E2E');
  ...
  try {
    ...
    const session = storedCodexSession(relaySession.session_id);
    const config = await requestConfig(relay, session.session_id);
    assert.equal(config?.capabilities?.file_browser, true, 'Codex config did not advertise file_browser');
    ...
    const result = {
      ...
      native_task_selection_changed: false,
...
if (require.main === module) {
  main().catch(error => {
    console.error(`VS Code Codex file-browser E2E: FAIL (${error.stack || error.message})`);
    process.exitCode = 1;
  });
}
